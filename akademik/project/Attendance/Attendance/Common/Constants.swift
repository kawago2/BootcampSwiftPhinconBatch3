...
enum FFirestore {
    ...
    static func addDataToSubcollectionWithAutoID(documentID: String, inCollection collection: String, subcollectionPath: String, data: [String: Any], completion: @escaping (Result<Void, Error>) -> Void) {
        let documentRef = db.collection(collection).document(documentID)
    ...
    static func deleteDataFromSubcollection(documentID: String, inCollection collection: String, subcollectionPath: String, autoGeneratedID: String, completion: @escaping (Result<Void, Error>) -> Void) {
        let documentRef = db.collection(collection).document(documentID)
        let subcollectionRef = documentRef.collection(subcollectionPath)
        let documentToDeleteRef = subcollectionRef.document(autoGeneratedID)

        documentToDeleteRef.delete { error in
            if let error = error {
                completion(.failure(error))
            } else {
                completion(.success(()))
