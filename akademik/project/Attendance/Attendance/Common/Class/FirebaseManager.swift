...
class FirebaseManager {
    ...
    func deleteDataFromSubcollectionWithAutoID(documentID: String, inCollection collection: String, subcollectionPath: String, autoGeneratedID: String, completion: @escaping (Result<Void, Error>) -> Void) {
        let db = Firestore.firestore()
        let documentRef = db.collection(collection).document(documentID)
        let subcollectionRef = documentRef.collection(subcollectionPath)
    ...
    func uploadImage(_ image: UIImage, toPath path: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        uploadTask.observe(.progress) { snapshot in
            let _ = 100.0 * Double(snapshot.progress!.completedUnitCount) / Double(snapshot.progress!.totalUnitCount)
        }
